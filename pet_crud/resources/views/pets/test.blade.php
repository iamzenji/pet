                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="breed" id="petBreed" class="form-select" required disabled>
...
$(document).on('click', '.edit-btn', function () {
    ...
    let petType  = $(this).data('type');
    let petBreed = $(this).data('breed');
    ...
    type   : 'GET',
    success: function(response) {
        let petTypeDropdown = $('#editPetForm select[name="type"]');
        petTypeDropdown.empty();
        petTypeDropdown.append('<option value="">Select Type</option>');

        response.forEach(type => {
            let selected = type === petType ? 'selected' : '';
            petTypeDropdown.append(`<option value="${type}" ${selected}>${type}</option>`);
        });

        // Fetch breeds after setting the type
        fetchBreeds(petType, petBreed);
    }
});
...
if (imagePath) {
    $('#editPetForm .text-center').html(`<img src="/storage/${imagePath}" width="100" class="img-thumbnail">`);
} else {
    $('#editPetForm .text-center').text('No Image');
}

$('#editModal').modal('show');
});

function fetchBreeds(petType, selectedBreed = '') {
if (!petType) return;

$.ajax({
    url: `/get-breeds/${petType}`,
    type: 'GET',
    success: function(response) {
        let breedDropdown = $('#editPetForm select[name="breed"]');
        breedDropdown.empty();
        breedDropdown.append('<option value="">Select Breed</option>');

        response.forEach(breed => {
            let selected = breed === selectedBreed ? 'selected' : '';
            breedDropdown.append(`<option value="${breed}" ${selected}>${breed}</option>`);
        });
...
$(document).on('change', '#editPetForm select[name="type"]', function () {
...
$(document).on('click', '.confirm-delete', function () {
  ...
  // Fetch pet types on add modal
$.get('/pets/types', function (data) {
    data.forEach(function (type) {
        $('#petType').append('<option value="' + type + '">' + type + '</option>');
    });
});

$('#petType').on('change', function () {
    let selectedType = $(this).val();
    $('#petBreed').empty().append('<option value="">Select Breed</option>').prop('disabled', true);

    if (selectedType) {
        $.get('/pets/breeds/' + selectedType, function (data) {
            data.forEach(function (breed) {
                $('#petBreed').append('<option value="' + breed + '">' + breed + '</option>');
            });
            $('#petBreed').prop('disabled', false);
        });
    }
});
